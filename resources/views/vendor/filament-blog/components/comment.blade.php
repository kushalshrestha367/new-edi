...
<form action="{{ route('filamentblog.comment.store', ['post' => $post->slug]) }}" method="POST" id="comments">
    ...
    <div class="border-t-2 py-10">
        ...
        <div>
            {{-- @dd(auth()->user()?->canComment()) --}}
            @if(auth()->user()?->canComment())
                <button type="submit"
                        class="g-recaptcha bg-{{ '['.@$site_setting->primary_color.']' ?? 'primary-800' }} hover:bg-primary-700 rounded-lg px-8 py-4 font-semibold text-white transition-all duration-300"
            ...
            @else
                <a
                        ...
                        class="bg-{{ '['.@$site_setting->primary_color.']' ?? 'primary-800' }} hover:bg-primary-700 rounded-lg px-8 py-4 font-semibold text-white transition-all duration-300">
                    <span>Login</span>
                </a>
            @endif
        </div>
